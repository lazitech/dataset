...
def generate_qa_pair(label: str, bg_color: str, fg_color: str) -> Dict[str, Any]:
    """
    Generates a structured dictionary containing the question, answer, and metadata.
    """
    ...
    if mode == "ask_fg":
        ...
        short_answer = f"{fg_color}, {bg_color}" # Simplified for metric checking
...
def generate_test_set(
    ...
    If balanced=True, it generates samples by iterating through all color combinations first.
    """
    if seed is not None:
        random.seed(seed)
    ...
    if balanced:
        # Permutations ensure we test (Red on Blue) AND (Blue on Red)
        # We explicitly exclude same-on-same
        color_pairs = list(itertools.permutations(COLOR_KEYS, 2))
        
        # If requested count is larger than pairs, we cycle; if smaller, we slice
        # But for a test set, usually we want at least one of each pair.
        if samples_total < len(color_pairs):
            print(f"Warning: Total samples ({samples_total}) is less than total unique color pairs ({len(color_pairs)}). Some pairs will be skipped.")
    ...
    for i, (bg_name, fg_name) in enumerate(selected_pairs):
        ...
        meta_entry = {
            ...
            "config": {
                "bg_color": bg_name,
                "fg_color": fg_name,
                "object_type": "shape" if label in SHAPE_LABELS else "text",
                "object_content": label,
            },
            "task": {
                "type": qa_data['mode'], # ask_bg, ask_fg, etc.
                "question": qa_data['question']
            },
            "ground_truth": {
                "short": qa_data['short_answer'],
                "full": qa_data['full_answer']
            }
        }
    ...
    print(f"Evaluation metadata saved to '{out_dir}/test_metadata.jsonl'")
...
def main() -> None:
    ...
    parser.add_argument("--count", type=int, default=240, help="Number of test samples") 
